...
w1,w2,w3,w4=16*[(0,1,0)],16*[(0,1,0)],16*[(0,1,0)],16*[(0,1,0)] #the lists used tokeep track of the robots movements
...
def encoder():
    ...
    while True:
        ...
        clkState1 = GPIO.input(clk1)    #first way of the encoders   
        dtState1 = GPIO.input(dt1)      #second way 
        ...
        if (time.time() - prev_time_enc)>=0.0005: # we update all values temporally
            
            w1=[(t1-t0, dir1, counter1)]+w1[:15]
            w2=[(t2-t0, dir2, counter2)]+w2[:15]
            w3=[(t3-t0, dir3, counter3)]+w3[:15]
            w4=[(t4-t0, dir4, counter4)]+w4[:15]
            prev_time_enc = time.time()
        

def return_speed():
    try:
        v1=(w1[0][2]-w1[-1][2])/(468.6*(w1[0][0]-w1[-1][0]))
    except:
        v1=0
    try:
        v2=(w2[0][2]-w2[-1][2])/(468.6*(w2[0][0]-w2[-1][0]))
    except:
        v2=0
    try:
        v3=(w3[0][2]-w3[-1][2])/(468.6*(w3[0][0]-w3[-1][0]))
    except:
        v3=0
    try:
        v4=(w4[0][2]-w4[-1][2])/(468.6*(w4[0][0]-w4[-1][0]))
    except:
        v4=0
